...
namespace GLTFHelpers
{
   ...
   void StoreValuesOfAttributeInAnimatedMesh(cgltf_attribute& attribute, cgltf_skin* skin, cgltf_node* nodes, unsigned int nodeCount, AnimatedMesh& outMesh)
   {
      ...
      for (unsigned int attributeValueIndex = 0; attributeValueIndex < numAttributeValues; ++attributeValueIndex)
      {
         ...
         case cgltf_attribute_type_joints:
         {
            ...
            glm::ivec4 jointIndices(static_cast<int>(attributeFloats[indexOfFirstFloatOfCurrAttribValue + 0] + 0.5f),
            ...
            jointIndices.x = std::max(0, GetNodeIndex(skin->joints[jointIndices.x], nodes, nodeCount));
            jointIndices.y = std::max(0, GetNodeIndex(skin->joints[jointIndices.y], nodes, nodeCount));
            jointIndices.z = std::max(0, GetNodeIndex(skin->joints[jointIndices.z], nodes, nodeCount));
            jointIndices.w = std::max(0, GetNodeIndex(skin->joints[jointIndices.w], nodes, nodeCount));

            influences.push_back(jointIndices);
         }
         break;
   ...
   void GetPositionsFromDracoMesh(const std::unique_ptr<draco::Mesh>& dracoMesh, std::vector<glm::vec3>& outPositions)
   {
      if (dracoMesh->NumNamedAttributes(draco::GeometryAttribute::POSITION) > 0)
      {
         auto posAttribute = dracoMesh->GetNamedAttribute(draco::GeometryAttribute::POSITION);
         if (posAttribute == NULL)
         {
            return;
         }

         if (posAttribute->data_type() == draco::DataType::DT_FLOAT32 &&
             posAttribute->num_components() == 3 &&
             posAttribute->byte_stride() <= 16) // TODO: Why is this condition necessary?
         {
            int numPositions = dracoMesh->num_points();
            outPositions.reserve(numPositions);
            for (int posIndex = 0; posIndex < numPositions; posIndex++)
            {
               float currPosition[3];
               posAttribute->GetMappedValue(draco::PointIndex(posIndex), &currPosition[0]);
               outPositions.push_back(glm::vec3(currPosition[0], currPosition[1], currPosition[2]));
            }
         }
      }
   }

   void GetNormalsFromDracoMesh(const std::unique_ptr<draco::Mesh>& dracoMesh, std::vector<glm::vec3>& outNormals)
   {
      if (dracoMesh->NumNamedAttributes(draco::GeometryAttribute::NORMAL) > 0)
      {
         ...
         if (normalAttribute->data_type() == draco::DataType::DT_FLOAT32 &&
             normalAttribute->num_components() == 3 &&
             normalAttribute->byte_stride() <= 16) // TODO: I'm not sure if this value is correct for normals
         {
            int numNormals = dracoMesh->num_points();
            outNormals.reserve(numNormals);
            for (int normalIndex = 0; normalIndex < numNormals; normalIndex++)
            {
               float currNormal[3];
               normalAttribute->GetMappedValue(draco::PointIndex(normalIndex), &currNormal[0]);
               glm::vec3 normal(currNormal[0], currNormal[1], currNormal[2]);
               ...
               outNormals.push_back(glm::normalize(normal));
            }
         }
      }
   }

   void GetTexCoordsFromDracoMesh(const std::unique_ptr<draco::Mesh>& dracoMesh, std::vector<glm::vec2>& outTexCoords)
   {
      if (dracoMesh->NumNamedAttributes(draco::GeometryAttribute::TEX_COORD) > 0)
      {
         ...
         if (texCoordAttribute->data_type() == draco::DataType::DT_FLOAT32 &&
             texCoordAttribute->num_components() == 2 &&
             texCoordAttribute->byte_stride() <= 16) // TODO: I'm not sure if this value is correct for texture coordinates
         {
            int numTexCoords = dracoMesh->num_points();
            outTexCoords.reserve(numTexCoords);
            for (int texCoordIndex = 0; texCoordIndex < numTexCoords; texCoordIndex++)
            {
               float currTexCoord[2];
               texCoordAttribute->GetMappedValue(draco::PointIndex(texCoordIndex), &currTexCoord[0]);
               outTexCoords.push_back(glm::vec2(currTexCoord[0], currTexCoord[1]));
            }
         }
      }
   }

   void GetWeightsFromDracoMesh(const std::unique_ptr<draco::Mesh>& dracoMesh, std::vector<glm::vec4>& outWeights)
   {
      int weightsAttributeID = -1;
      int numGenericAttributes = dracoMesh->NumNamedAttributes(draco::GeometryAttribute::GENERIC);
      for (int genericAttributeIndex = 0; genericAttributeIndex < numGenericAttributes; genericAttributeIndex++)
      {
         int currGenericAttributeID = dracoMesh->GetNamedAttributeId(draco::GeometryAttribute::GENERIC, genericAttributeIndex);
         const draco::PointAttribute* currGenericAttribute = dracoMesh->GetAttributeByUniqueId(currGenericAttributeID);
         if (currGenericAttribute->data_type() == draco::DataType::DT_FLOAT32 &&
             currGenericAttribute->num_components() == 4 &&
             currGenericAttribute->byte_stride() <= 16) // TODO: I'm not sure if this value is correct for weights
         {
            weightsAttributeID = currGenericAttributeID;
            break;
         }
      }

      if (weightsAttributeID == -1)
      {
         return;
      }

      auto weightsAttribute = dracoMesh->GetAttributeByUniqueId(weightsAttributeID);
      int numWeights = dracoMesh->num_points();
      outWeights.reserve(numWeights);
      for (int weightIndex = 0; weightIndex < numWeights; weightIndex++)
      {
         float currWeight[4];
         weightsAttribute->GetMappedValue(draco::PointIndex(weightIndex), &currWeight[0]);
         outWeights.push_back(glm::vec4(currWeight[0], currWeight[1], currWeight[2], currWeight[3]));
      }
   }

   void GetInfluencesFromDracoMesh(const std::unique_ptr<draco::Mesh>& dracoMesh, cgltf_skin* skin, cgltf_node* nodes, unsigned int nodeCount, std::vector<glm::ivec4>& outInfluences)
   {
      int influencesAttributeID = -1;
      int numGenericAttributes = dracoMesh->NumNamedAttributes(draco::GeometryAttribute::GENERIC);
      for (int genericAttributeIndex = 0; genericAttributeIndex < numGenericAttributes; genericAttributeIndex++)
      {
         int currGenericAttributeID = dracoMesh->GetNamedAttributeId(draco::GeometryAttribute::GENERIC, genericAttributeIndex);
         const draco::PointAttribute* currGenericAttribute = dracoMesh->GetAttributeByUniqueId(currGenericAttributeID);
         if (currGenericAttribute->data_type() == draco::DataType::DT_UINT8 &&
             currGenericAttribute->num_components() == 4 &&
             currGenericAttribute->byte_stride() <= 4) // TODO: I'm not sure if this value is correct for influences
         {
            influencesAttributeID = currGenericAttributeID;
            break;
         }
      }

      if (influencesAttributeID == -1)
      {
         return;
      }

      auto influencesAttribute = dracoMesh->GetAttributeByUniqueId(influencesAttributeID);
      int numInfluences = dracoMesh->num_points();
      outInfluences.reserve(numInfluences);
      for (int influenceIndex = 0; influenceIndex < numInfluences; influenceIndex++)
      {
         uint8_t currInfluence[4];
         influencesAttribute->GetMappedValue(draco::PointIndex(influenceIndex), &currInfluence[0]);
         ...
         // not indices into the array of nodes of the glTF file, so we need to convert them from being
         // skin-relative to being file-relative
         glm::ivec4 jointIndices(currInfluence[0], currInfluence[1], currInfluence[2], currInfluence[3]);
         ...
         outInfluences.push_back(jointIndices);
      }
   }
}
...
std::vector<AnimatedMesh> LoadDracoAnimatedMeshes(cgltf_data* data)
{
   ...
   for (unsigned int nodeIndex = 0; nodeIndex < numNodes; ++nodeIndex)
   {
      ...
      for (unsigned int primitiveIndex = 0; primitiveIndex < numPrimitives; ++primitiveIndex)
      {
         ...
         if (currPrimitive->has_draco_mesh_compression)
         {
            if (currPrimitive->draco_mesh_compression.buffer_view->buffer->data)
            {
               char*  compressedData     = static_cast<char*>(currPrimitive->draco_mesh_compression.buffer_view->buffer->data);
               size_t compressedDataSize = currPrimitive->draco_mesh_compression.buffer_view->buffer->size;

               draco::DecoderBuffer decoderBuffer;
               decoderBuffer.Init(compressedData, compressedDataSize);

               auto statusOrGeomType = draco::Decoder::GetEncodedGeometryType(&decoderBuffer);
               if (!statusOrGeomType.ok())
               {
                  return animatedMeshes;
               }

               const draco::EncodedGeometryType geomType = statusOrGeomType.value();
               if (geomType == draco::TRIANGULAR_MESH)
               {
                  draco::Decoder decoder;
                  auto statusOrMesh = decoder.DecodeMeshFromBuffer(&decoderBuffer);
                  if (!statusOrMesh.ok())
                  {
                     return animatedMeshes;
                  }
                  ...
                  GLTFHelpers::GetPositionsFromDracoMesh(dracoMesh, currMesh.GetPositions());
                  GLTFHelpers::GetNormalsFromDracoMesh(dracoMesh, currMesh.GetNormals());
                  GLTFHelpers::GetTexCoordsFromDracoMesh(dracoMesh, currMesh.GetTexCoords());
                  GLTFHelpers::GetWeightsFromDracoMesh(dracoMesh, currMesh.GetWeights());
                  GLTFHelpers::GetInfluencesFromDracoMesh(dracoMesh, currNode->skin, data->nodes, numNodes, currMesh.GetInfluences());

                  int numTriangles = dracoMesh->num_faces();
                  std::vector<unsigned int>& indices = currMesh.GetIndices();
                  indices.reserve(numTriangles * 3);
                  for (int triangleIndex = 0; triangleIndex < numTriangles; ++triangleIndex)
                  {
                     indices.push_back(dracoMesh->face(draco::FaceIndex(triangleIndex))[0].value());
                     indices.push_back(dracoMesh->face(draco::FaceIndex(triangleIndex))[1].value());
                     indices.push_back(dracoMesh->face(draco::FaceIndex(triangleIndex))[2].value());
                  }
               }
            }
         }
