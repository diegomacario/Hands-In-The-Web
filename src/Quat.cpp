...
{
   glm::vec3 nAxis = Utility::normalizeWithZeroLengthCheck(axis);
...
{
   return Utility::normalizeWithZeroLengthCheck(glm::vec3(q.x, q.y, q.z));
}

Q::quat Q::fromTo(const glm::vec3& from, const glm::vec3& to)
{
   glm::vec3 nFrom = Utility::normalizeWithZeroLengthCheck(from);
   glm::vec3 nTo   = Utility::normalizeWithZeroLengthCheck(to);

   if (glm::length2(nFrom - nTo) < Utility::epsilon) // nFrom == nTo
   {
      // If from and to are the same, then we return a unit quaternion
      return Q::quat();
   }
   else if (glm::length2(nFrom - (nTo * -1.0f)) < Utility::epsilon) // nFrom == -nTo
   {
      ...
      glm::vec3 axisOfRot = Utility::normalizeWithZeroLengthCheck(glm::cross(nFrom, mostOrthogBasis));
   ...
   glm::vec3 halfVecBetweenFromAndTo = Utility::normalizeWithZeroLengthCheck(nFrom + nTo);
...
Q::quat Q::operator^(const Q::quat& q, float exponent)
{
   ...
   glm::vec3 axisOfRot = Utility::normalizeWithZeroLengthCheck(q.vector);
...
{
   return (glm::abs(a.x - b.x) <= Utility::epsilon &&
           glm::abs(a.y - b.y) <= Utility::epsilon &&
           glm::abs(a.z - b.z) <= Utility::epsilon &&
           glm::abs(a.w - b.w) <= Utility::epsilon);
}
...
bool Q::sameOrientation(const Q::quat& a, const Q::quat& b)
{
   ...
   return (glm::abs(a.x - b.x) <= Utility::epsilon &&
           ...
           glm::abs(a.z - b.z) <= Utility::epsilon &&
           glm::abs(a.w - b.w) <= Utility::epsilon) ||
          (glm::abs(a.x + b.x) <= Utility::epsilon &&
           glm::abs(a.y + b.y) <= Utility::epsilon &&
           glm::abs(a.z + b.z) <= Utility::epsilon &&
           glm::abs(a.w + b.w) <= Utility::epsilon);
}
...
Q::quat Q::slerp(const Q::quat& start, const Q::quat& end, float t)
{
   ...
   if (glm::abs(dot(start, end)) > 1.0f - Utility::epsilon)
   {
...
Q::quat Q::lookRotation(const glm::vec3& direction, const glm::vec3& upReference)
{
   ...
   glm::vec3 fwd   = Utility::normalizeWithZeroLengthCheck(direction);
   glm::vec3 up    = Utility::normalizeWithZeroLengthCheck(upReference);
   glm::vec3 right = Utility::normalizeWithZeroLengthCheck(glm::cross(up, fwd));
...
Q::quat Q::mat4ToQuat(const glm::mat4& m)
{
   ...
   glm::vec3 up      = Utility::normalizeWithZeroLengthCheck(glm::vec3(m[1].x, m[1].y, m[1].z));
   glm::vec3 forward = Utility::normalizeWithZeroLengthCheck(glm::vec3(m[2].x, m[2].y, m[2].z));
