...
#ifdef ENABLE_IMGUI
      // The cursor has been released by the game, so we tell imgui to start processing it
...
#ifdef ENABLE_IMGUI
      // The cursor has been captured by the game, so we tell imgui to stop processing it
      ImGui::GetIO().ConfigFlags |= (ImGuiConfigFlags_NoMouseCursorChange);
      ImGui::GetIO().ConfigFlags |= (ImGuiConfigFlags_NoMouse);
#endif
   }
...
void Window::updateWindowDimensions(int width, int height)
{
   ...
   // TODO: Is the -1 still necessary?
   // When a user rotates their phone to either portrait or landscape updateWindowDimensions gets called
   // The call below to glfwSetWindowSize is done with basically inverted width and height values
   // It seems that this call is a no-op unless the window dimensions actually change, so what ends up
   // happening is that after the phone is rotated the call below does nothing and the framebufferSizeCallback is never invoked
   // To get around this we subtract 1 pixel from the height which results in the callback being invoked
   glfwSetWindowSize(mWindow, mWidthOfWindowInPix, mHeightOfWindowInPix - 1);
}
#endif
